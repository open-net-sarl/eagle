...
class account_invoice( osv.osv ):
    ...
    def create(self, cr, uid, vals, context=None):
        ret = super( account_invoice, self).create( cr, uid, vals, context=context )

        if vals.get('contract_id', False):
            contract_id = vals['contract_id']
            inv_l_ids = vals.get('invoice_line',[])
            if inv_l_ids and inv_l_ids[0] and inv_l_ids[0][2]:
                self.pool.get( 'account.invoice.line' ).write( cr, uid, inv_l_ids[0][2], {'contract_id':contract_id}, context=context )
